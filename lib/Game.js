...
Game.prototype.checkEndOfBattle = function() {
    ...
    if (this.player.isAlive() && this.currentEnemy.isAlive()) {
        ...
        // call battle() function to start another round
        this.battle();
    }

    // player is alive but enemy is not
    else if (this.player.isAlive() && !this.currentEnemy.isAlive()) {
        console.log(`You've defeated the ${this.currentEnemy.name}`);
        // player obtains enemy's unused potions
        this.player.addPotion(this.currentEnemy.potion);
        console.log(`${this.player.name} found a ${this.currentEnemy.potion.name} potion`);
        // go to next round
        this.roundNumber++;
        // if there are enemies left, go to next round
        if (this.roundNumber < this.enemies.length) {
            // assign current enemy an index equal to the current round number
            this.currentEnemy = this.enemies[this.roundNumber];
            // start new battle with next enemy
            this.startNewBattle();
        }
        // if no enemies are left, you win the game
        else {
            console.log('You win!');
        }
    }

    // enemy is alive but player is not
    else {
